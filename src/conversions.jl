...
PauliSum(p::Pauli{N,W}; T=ComplexF64) where {N,W} = PauliSum{N, W, T}(PauliBasis(p)=>coeff(p))
PauliSum(p::PauliBasis{N,W}; T=ComplexF64) where {N,W} = PauliSum{N, W, T}(PauliBasis(p)=>coeff(p))
